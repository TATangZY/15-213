...
void eval(char* cmdline)
{
    ...
    if (!builtin_cmd(argv)) { // 如果不是 build-in 程序再执行接下来的步骤
        sigset_t mask;
        sigset_t pre_mask;
        sigset_t mask_all;
        Sigfillset(&mask_all);
...
int builtin_cmd(char** argv)
{
    ...
    int flag = 0;
    flag |= strcmp(argv[0], "quit") ? 0 : 1;
    flag |= strcmp(argv[0], "jobs") ? 0 : 2;
    flag |= strcmp(argv[0], "bg") ? 0 : 3;
    flag |= strcmp(argv[0], "fg") ? 0 : 4;
    // int flag = isnt_quit && isnt_jobs && isnt_bg && isnt_or && isnt_fg;
...
void do_bgfg(char** argv)
{
    ...
    if (!strcmp(argv[0], "bg")) {
        job->state = BG;
        printf("[%d] (%d) %s", job->jid, jobs->pid, job->cmdline);
